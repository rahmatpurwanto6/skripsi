        ...
        <div class="col-md-12">
            <div class="box box-info">
                <div class="box-header with-border">
                    ...
                    <a href="<?= base_url('pengajuan_admin1/tambah_kemhs'); ?>" class="btn btn-primary" style="margin-top: 1px;"><i class="fa fa-plus"></i> Pengajuan untuk kemahasiswaan</a>
                        ...
                        <tbody>
                            ...
                            foreach ($tampil as $row) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <!-- detail admin1 -->
                                        <a href="<?= base_url('pengajuan_admin1/detail/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-info btn-sm"> <i class="fa fa-eye"></i> </a>
                                        <!-- end detail admin1 -->

                                        <!-- button batal -->
                                        <a href="<?= base_url('pengajuan_admin1/batal/') . encrypt_url($row['id_pinjam']); ?>" <?php if ($row['status'] == 1 || $row['status'] == 2) { ?> echo style="display: none;" <?php } ?> class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin akan membatalkannya?');"><i class="fa fa-close"></i> </a>
                                        <!-- end button batal -->

                                        <!-- print superadmin -->
                                        <?php if ($row['status'] == 1) { ?>

                                            <!-- <a href="<?= base_url('pengajuan_admin1/cetak_ormawa/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-primary btn-sm" target="_blank"> <i class="fa fa-print"></i></a> -->

                                            <a href="<?= base_url('pengajuan_admin1/cetak_laporan/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-primary btn-sm" target="_blank"> <i class="fa fa-print"></i></a>

                                        <?php } ?>
                                        <!-- end print superadmin -->

                                        <a href="<?= base_url('pengajuan_admin1/hapus/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin akan menghapusnya?');"> <i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<script>
    ...
    function load_data() {
        $("#tabel3").DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('pengajuan_admin1/get_data_admin1') ?>",
                "type": "POST"
            },
