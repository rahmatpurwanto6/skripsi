                            ...
                            foreach ($tampil as $row) : ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if ($role_id == 1) : ?>
                                            <a href="<?= base_url('persetujuan/detail_persetujuan/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-info btn-sm"> <i class="fa fa-eye"></i> </a>
                                        <?php endif; ?>

                                        <!-- end dosen dan mhs -->
                                        <!-- end detail superadmin -->

                                        <!-- detail kemahasiswaan -->
                                        <?php
                                        $role_id = $this->session->userdata('role_id');
                                        if ($role_id == 4) : ?>
                                            <a href="<?= base_url('persetujuan_kemhs/detail_kemhs/') . encrypt_url($row['id_pinjam']); ?>" class="btn btn-info btn-sm"> <i class="fa fa-eye"></i> </a>
                                        <?php endif; ?>
                                        <!-- end detail kemahasiswaan -->
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<script>
    ...
    function load_data() {
        $("#tabel1").DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('persetujuan/get_persetujuanadmin') ?>",
                "type": "POST"
            },
