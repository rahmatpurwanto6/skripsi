...
class Pengajuan_dosen extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        //Do your magic here
        $this->load->model(['Pengajuan_m', 'Pengajuan_dosen_m', 'Kelas_m', 'Jurusan_m', 'Ruangan_m']);
    ...
    public function index()
    {
        ...
        $data['title'] = 'Halaman Menu Pengajuan Peminjaman Dosen';
        $data['tampil'] = $this->Pengajuan_m->getdatadosen();
        $data['notif'] = $this->Pengajuan_m->total_notif();
        $this->template->load('template', 'pengajuan/pengajuandosen/index', $data);
    }

    public function get_datadosen()
    {
        $list = $this->Pengajuan_dosen_m->get_datatables();
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = '<a href="' . base_url('pengajuan_dosen/detail/' . encrypt_url(($item->id_pinjam))) . '" class="btn btn-info btn-sm">
            <li class="fa fa-eye">
            </li></a>
            <a href="' . base_url('pengajuan_dosen/batal/' . encrypt_url(($item->id_pinjam))) . '" ' . ($item->status == 1 || $item->status == 2 ? 'style="display:none;' : '') . ' id="batal" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah anda akan membatalkannya?\');">
            <li class="fa fa-close">
            </li></a> <a href="' . base_url('pengajuan_dosen/cetak_laporan/' . encrypt_url(($item->id_pinjam))) . '" ' . ($item->status == 0 || $item->status == 2 ? 'style="display:none;' : '') . ' class="btn btn-primary btn-sm" target="_blank">
            <li class="fa fa-print">
            </li></a> 
        <a href="' . base_url('pengajuan_dosen/hapus/' . encrypt_url(($item->id_pinjam))) . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah anda yakin?\');">
        ...
        $output = array(
            "draw" => @$_POST['draw'],
            "recordsTotal" => $this->Pengajuan_dosen_m->count_all(),
            "recordsFiltered" => $this->Pengajuan_dosen_m->count_filtered(),
            "data" => $data,
        );
    ...
    public function detail($id)
    {
        ...
        $this->template->load('template', 'pengajuan/pengajuandosen/detail/detail', $data);
    }

    public function tambah()
    {
        $data['title'] = 'Halaman Pengajuan Peminjaman Dosen';
        $data['kampus'] = $this->Pengajuan_m->getkampus();
        $this->template->load('template', 'pengajuan/pengajuandosen/tambah', $data);
    }
            ...
            if ($cek) {
                ...
                if ($kampus == $row['kampus'] and $ruangan == $row['id_ruangan'] and $date1 == $row['tgl1'] and $date2 == $row['tgl2'] and strtotime($mulai) == strtotime($row['mulai']) and strtotime($selesai) == strtotime($row['selesai'])) {
                    echo "<script> 
                                ...
                                window.location='" . base_url('pengajuan_dosen/tambah') . "';
                                </script>";
                }
            } else {
                $data = [
    ...
    public function hapus($id)
    {
        $id = decrypt_url($id);
        $this->Pengajuan_m->cancel($id);
        $this->session->set_flashdata('pesan', 'Pengajuan berhasil hapus');
        redirect('pengajuan_dosen');
    }
