...
class Pengajuan_mhs extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        //Do your magic here
        $this->load->model(['Pengajuan_m', 'Pengajuan_mhs_m', 'Ruangan_m']);
    ...
    public function index()
    {
        ...
        $data['title'] = 'Halaman Menu Pengajuan Peminjaman Mahasiswa';
        $data['tampil'] = $this->Pengajuan_m->getdatamhs();
        $data['notif'] = $this->Pengajuan_m->total_notif();
        $this->template->load('template', 'pengajuan/pengajuan_mhs/index', $data);
    }

    public function get_datamhs()
    {
        $list = $this->Pengajuan_mhs_m->get_datatables();
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = '<a href="' . base_url('pengajuan_mhs/detail/' . encrypt_url(($item->id_pinjam))) . '" class="btn btn-info btn-sm">
            <li class="fa fa-eye">
            </li></a>
            <a href="' . base_url('pengajuan_mhs/batal/' . encrypt_url(($item->id_pinjam))) . '" ' . ($item->status == 1 || $item->status == 2 ? 'style="display:none;' : '') . ' id="batal" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah anda akan membatalkannya?\');">
            <li class="fa fa-close">
            </li></a> <a href="' . base_url('pengajuan_mhs/cetak_laporan/' . encrypt_url(($item->id_pinjam))) . '" ' . ($item->status == 0 || $item->status == 2 ? 'style="display:none;' : '') . ' class="btn btn-primary btn-sm" target="_blank">
            <li class="fa fa-print">
            </li></a> 
        <a href="' . base_url('pengajuan_mhs/hapus/' . encrypt_url(($item->id_pinjam))) . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah anda yakin?\');">
        ...
        $output = array(
            "draw" => @$_POST['draw'],
            "recordsTotal" => $this->Pengajuan_mhs_m->count_all(),
            "recordsFiltered" => $this->Pengajuan_mhs_m->count_filtered(),
            "data" => $data,
        );
    ...
    public function detail($id)
    {
        ...
        $this->template->load('template', 'pengajuan/pengajuan_mhs/detail/detail', $data);
    }

    public function tambah()
    {
        $data['title'] = 'Halaman Pengajuan Peminjaman Mahasiswa';
        $data['kampus'] = $this->Pengajuan_m->getkampus();
        $this->template->load('template', 'pengajuan/pengajuan_mhs/tambah', $data);
    }
            ...
            if ($cek) {
                ...
                if ($kampus == $row['kampus'] and $ruangan == $row['id_ruangan'] and $date1 == $row['tgl1'] and $date2 == $row['tgl2'] and strtotime($mulai) == strtotime($row['mulai']) and strtotime($selesai) == strtotime($row['selesai'])) {
                    echo "<script> 
                                ...
                                window.location='" . base_url('pengajuan_mhs/tambah') . "';
                                </script>";
                }
            } else {
                $data = [
