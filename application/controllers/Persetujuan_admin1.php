...
class Persetujuan_admin1 extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        //Do your magic here
        $this->load->model(['Persetujuan_m', 'Pengajuan_m', 'Persetujuan_admin1_m']);
    ...
    public function index()
    {
        ...
        $data['title'] = 'Halaman Menu Persetujuan Akademik Kampus 1';
        $data['tampil'] = $this->Persetujuan_m->getadmin1();
        $data['notif'] = $this->Pengajuan_m->total_notif();
        $this->template->load('template', 'persetujuan/admin1/index', $data);
    }

    public function get_persetujuanadmin1()
    {
        $list = $this->Persetujuan_admin1_m->get_datatables();
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = '<a href="' . base_url('persetujuan_admin1/detail_persetujuan/' . encrypt_url(($item->id_pinjam))) . '" class="btn btn-info btn-sm">
            <li class="fa fa-eye">
            </li></a>';
        ...
        $output = array(
            "draw" => @$_POST['draw'],
            "recordsTotal" => $this->Persetujuan_admin1_m->count_all(),
            "recordsFiltered" => $this->Persetujuan_admin1_m->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $id = decrypt_url($id);
        $data['title'] = 'Halaman Persetujuan Akademik Kampus 1';
        $data['tampil'] = $this->Persetujuan_m->detail_kampus1($id);
        $this->template->load('template', 'persetujuan/admin1/detail/detail_persetujuan', $data);
    }
    ...
    public function simpanpersetujuan()
    {
        ...
        if ($acc) {
            ...
            if ($row['acc_kmhs'] == 0) {
                ...
                window.location='" . base_url('persetujuan_admin1') . "';
                </script>";
            } else {
                if ($aksi == 2) {
                    $data = [
    ...
    //     $this->template->load('template', 'persetujuan/admin1/detail/persetujuan_ormawa_akademik1', $data);
    // }
...
/* End of file Persetujuan_admin1.php */
